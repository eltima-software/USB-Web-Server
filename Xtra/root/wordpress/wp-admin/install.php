...
if(isset($_GET['first'])){
	...
	if($createdb){
		$id = 1;
		while(mysql_num_rows(mysql_query("SELECT user FROM mysql.user WHERE user='wordpress".$id."'")) == 1){
			$id++;
		}

		mysql_query("CREATE USER 'wordpress".$id."'@'localhost' IDENTIFIED BY 'usbw'") or die("Could not make user wordpress".$id.":<br>".mysql_error()); // Create new user: 'wordpress'
		mysql_query("CREATE DATABASE IF NOT EXISTS `wordpress".$id."`") or die("Could not make database wordpress".$id.":<br>". mysql_error()); // Create database 'wordpress'
		mysql_query("GRANT ALL PRIVILEGES ON `wordpress".$id."`.* TO 'wordpress".$id."'@'localhost'") or die("Could not grant user wordpress".$id.":<br>". mysql_error()); // Grant user 'wordpress' for database 'wordpress'
		
		$string='
			<?php
			define("DB_NAME", "wordpress'.$id.'");
			define("DB_USER", "wordpress'.$id.'");
		...
		echo '<form action="install.php?first" method="POST">Root password :<input type="text" name="pw"><br><input type="submit" name="changepw" value="Go"></form>';
		exit();
	}
